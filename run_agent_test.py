...
if __name__ == "__main__":
    ...
    while True:
        state = playwriter.graph.get_state(graph_config)
        if not state.next:
            print("Graph paused state:",state)
            user_input = {"data": input("Enter answer")}
            resume_input = Command(resume= user_input)
            print_stream(playwriter.graph.stream(input =resume_input,  # type: ignore
                                                 stream_mode= "values", 
                                                 config = graph_config))
        else:
            print("Done")
